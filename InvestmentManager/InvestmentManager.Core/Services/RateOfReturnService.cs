...
namespace InvestmentManager.Core.Services
{
    ...
    {
        public RateOfReturnService(IAccountMarketValueRepository marketValueRepository, IAccountCashFlowsRepository cashFlowRepository)
        {
            this.marketValueRepository = marketValueRepository;
            this.cashFlowRepository = cashFlowRepository;
        }

        private readonly IAccountMarketValueRepository marketValueRepository;
        private readonly IAccountCashFlowsRepository cashFlowRepository;
        ...
        internal List<PeriodInfo> GetMonthlySummaries(List<AccountMarketValue> marketValues, List<CashFlow> cashFlows)
        {
            ...
            var perfMarketValues = marketValues
                .Where(mv => (mv.TradeDate != null ? mv.TradeDate.IsMonthEnd : false)
            ...
            for (int i = 1; i < perfMarketValues.Count; i++)
            {
                ...
                PeriodInfo period = new PeriodInfo()
                {
                    ...
                    EndingDate = (isLastPeriod && (endingMarketValue.TradeDate != null ? !endingMarketValue.TradeDate.IsMonthEnd : false))
